...
public class ElasticHelper {
  ...
  public static CheckedSupplier<SearchRequest, ProvenanceException> scrollingSearchRequest(String index, String docType,
    int pageSize) {
    return () -> {
  ...
  public static CheckedFunction<SearchRequest, SearchRequest, ProvenanceException> sortByTimestamp(SortOrder order) {
    return (SearchRequest sr) -> {
      sr.source().sort(ProvElasticFields.FileOpsBase.TIMESTAMP.toString().toLowerCase(), order);
      return sr;
    };
  }
  
  public static CheckedSupplier<SearchRequest, ProvenanceException> baseSearchRequest(String index, String docType,
    int pageSize) {
    return () -> {
      SearchRequest sr = new SearchRequest(index)
        .types(docType);
  ...
  public static CheckedSupplier<SearchRequest, ProvenanceException> countSearchRequest(String index, String docType) {
    return () -> {
      SearchRequest sr = new SearchRequest(index)
        .types(docType);
      sr.source().size(0);
  ...
  public static CheckedFunction<SearchRequest, SearchRequest, ProvenanceException> withPagination(
    Integer offset, Integer limit, Integer defaultPageSize) {
    return (SearchRequest sr) -> {
      try {
