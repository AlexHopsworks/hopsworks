...
public class ProvFileOpDTO {
  ...
  public static class CompactByFile {
    ...
    public CompactByFile(ProvOpsElastic op) {
      this.inodeOperation = op.getInodeOperation().name();
      this.appId = op.getAppId();
      this.userId = op.getUserId();
      this.logicalTime = op.getLogicalTime();
  ...
  public static class File {
    ...
    public void addOp(ProvOpsElastic op) throws ProvenanceException {
      if(!op.getInodeId().equals(this.inodeId)) {
        throw new ProvenanceException(RESTCodes.ProvenanceErrorCode.BAD_REQUEST, Level.INFO,
          "op mismatch - inodeId");
      }
    ...
    public static File compact(List<ProvOpsElastic> ops) throws ProvenanceException {
      if(ops.isEmpty()) {
        throw new ProvenanceException(RESTCodes.ProvenanceErrorCode.BAD_REQUEST, Level.INFO,
          "cannot create a FileOps object out of no ops");
      }
      Iterator<ProvOpsElastic> it = ops.iterator();
  ...
  public static class PList {
    List<ProvOpsElastic> items;
    Long count;
    
    public PList() {}
    
    public PList(List<ProvOpsElastic> items, Long count) {
      this.items = items;
      this.count = count;
    }
    
    public List<ProvOpsElastic> getItems() {
      return items;
    }
    
    public void setItems(List<ProvOpsElastic> items) {
      this.items = items;
    }
  ...
  public static class ArtifactFile {
    ...
    public void addCreate(ProvOpsElastic createOp) {
      init(createOp);
      this.createOp = new Base(createOp);
    }
  
    public void addDelete(ProvOpsElastic deleteOp) {
      init(deleteOp);
      this.deleteOp = new Base(deleteOp);
    }
  
    public void addFirstRead(ProvOpsElastic readOp, Long readCount) {
      init(readOp);
      this.firstReadOp = new Base(readOp);
      this.readCount = readCount;
    }
    public void addFirstAppend(ProvOpsElastic appendOp, Long appendCount) {
      init(appendOp);
      this.firstAppendOp = new Base(appendOp);
      this.appendCount = appendCount;
    }
    
    private void init(ProvOpsElastic op) {
      if(parentId == null) {
  ...
  public static class ArtifactBase {
    ...
    public ProvParser.DocSubType getMlType() {
      return mlType;
    }
  
    public void setMlType(ProvParser.DocSubType mlType) {
      this.mlType = mlType;
    }
  }
