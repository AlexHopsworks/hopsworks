...
public class ProvFileOpDTO {
  ...
  public static class Base {
    ...
    public Base(ProvFileOpElastic op) {
      this.inodeId = op.getInodeId();
      this.inodeOperation = op.getInodeOperation().name();
  ...
  public static class CompactByFile {
    ...
    public CompactByFile(ProvFileOpElastic op) {
      this.inodeOperation = op.getInodeOperation().name();
      this.appId = op.getAppId();
      this.userId = op.getUserId();
      this.logicalTime = op.getLogicalTime();
  ...
  public static class File {
    ...
    public File(ProvFileOpElastic op){
      this.inodeId = op.getInodeId();
      this.parentInodeId = op.getParentInodeId();
      this.projectInodeId = op.getProjectInodeId();
      this.inodeName = op.getInodeName();
    ...
    public void addOp(Base op) throws ProvenanceException {
    ...
    public static File compact(List<ProvFileOpElastic> ops) throws ProvenanceException {
      if(ops.isEmpty()) {
        throw new ProvenanceException(RESTCodes.ProvenanceErrorCode.BAD_REQUEST, Level.INFO,
          "cannot create a FileOps object out of no ops");
      }
      Iterator<ProvFileOpElastic> it = ops.iterator();
  ...
  public static class PList {
    List<ProvFileOpElastic> items;
    Long count;
    
    public PList() {}
    
    public PList(List<ProvFileOpElastic> items, Long count) {
      this.items = items;
      this.count = count;
    }
    
    public List<ProvFileOpElastic> getItems() {
      return items;
    }
    
    public void setItems(List<ProvFileOpElastic> items) {
      this.items = items;
    }
  ...
  public static class Count {
    ...
    public static Count instance(Pair<Long, Map<ProvElasticAggregations.ProvAggregations, List>> elasticResult)
      ...
      for(Map.Entry<ProvElasticAggregations.ProvAggregations, List> aggregation :
        elasticResult.getValue1().entrySet()) {
        switch(aggregation.getKey()) {
          case FILES_IN:
            filesInProject = new CountAux(aggregation.getValue());
            break;
          case FILES_LEAST_ACTIVE_BY_LAST_ACCESSED:
            filesLeastAccessedByLastAccessed = new CountAux(aggregation.getValue());
            break;
          case PROJECTS_LEAST_ACTIVE_BY_LAST_ACCESSED:
            projectLeastAccessedByLastAccessed = new CountAux(aggregation.getValue());
            break;
          case ARTIFACT_FOOTPRINT:
            artifactFootprint = new CountAux(aggregation.getValue());
            break;
          default:
            throw new ProvenanceException(RESTCodes.ProvenanceErrorCode.UNSUPPORTED, Level.INFO,
              "aggregation:" + aggregation.getKey().toString() + " not handled");
        }
      }
  ...
  public static class ArtifactFile {
    ...
    public void addCreate(ProvFileOpElastic createOp) {
      init(createOp);
      this.createOp = new Base(createOp);
    }
  
    public void addDelete(ProvFileOpElastic deleteOp) {
      init(deleteOp);
      this.deleteOp = new Base(deleteOp);
    }
  
    public void addFirstRead(ProvFileOpElastic readOp, Long readCount) {
      init(readOp);
      this.firstReadOp = new Base(readOp);
      this.readCount = readCount;
    }
    public void addFirstAppend(ProvFileOpElastic appendOp, Long appendCount) {
      init(appendOp);
      this.firstAppendOp = new Base(appendOp);
      this.appendCount = appendCount;
    }
    
    private void init(ProvFileOpElastic op) {
      if(parentId == null) {
  ...
  public static class ArtifactBase {
    ...
    private String mlId;
    private ProvElasticFields.DocSubType mlType;
  
    public ArtifactBase() {
    }
  
    public ArtifactBase(Long projectInodeId, Long datasetInodeId, String mlId,
      ProvElasticFields.DocSubType mlType) {
      this.projectInodeId = projectInodeId;
    ...
    public ProvElasticFields.DocSubType getMlType() {
      return mlType;
    }
  
    public void setMlType(ProvElasticFields.DocSubType mlType) {
      this.mlType = mlType;
    }
  }
