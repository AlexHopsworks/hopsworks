...
public class ProvExecution {
  ...
  public static ElasticHitsHandler<ProvOpsElastic, Footprint<String, ProvMLStateMinDTO>, ?, ProvenanceException>
    mlStateProc() {
    return ElasticHitsHandler.instanceBasic(new ProvExecution.Footprint<>(),
      (BasicElasticHit hit) -> ProvOpsElastic.instance(hit),
      (ProvOpsElastic op, Footprint<String, ProvMLStateMinDTO> state) -> {
  ...
  public static Map<Long, ProvStateMinDTO> processFootprint(List<ProvOpsElastic> fileOps,
    Provenance.FootprintType footprintType) {
    Footprint<Long, ProvStateMinDTO> rawFootprint = new Footprint<>();
    for(ProvOpsElastic fileOp : fileOps) {
      Provenance.MLType mlType = ProvParser.mlTypeParser(fileOp.getDocSubType());
      ProvStateMinDTO fileState = ProvStateMinDTO.fromFileOp(fileOp, mlType);
      addToFootprint(rawFootprint, fileState, fileOp);
    }
    Map<Long, ProvStateMinDTO> footprint = processFootprint(rawFootprint, footprintType);
    return footprint;
  }
  ...
  private void processMLFootprint(Footprint<String, ProvMLStateMinDTO> footprint, ProvOpsElastic fileOp) {
